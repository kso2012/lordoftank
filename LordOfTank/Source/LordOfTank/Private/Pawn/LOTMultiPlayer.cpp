...
void ALOTMultiPlayer::FireEnd()
{
	...
	if (CurInventoryIndex == P_COMMON || HadProjectileNum[CurInventoryIndex] > 0) {
		...
		if (CurrentProjectile != NULL && bIsPushFire && GameModeTest->bIsMyTurn && !bIsShoot && bIsMyTurn)
		{
			...
			if (CurInventoryIndex == P_COMMON) {
				GEngine->AddOnScreenDebugMessage(2, 2.0f, FColor::Blue, FString::Printf(TEXT("일반탄 발사")));
			}
			else if (CurInventoryIndex == P_ARMORPIERCE) {
				GEngine->AddOnScreenDebugMessage(2, 2.0f, FColor::Blue, FString::Printf(TEXT("관통탄 발사 %d발 남음"), HadProjectileNum[CurInventoryIndex]));
			}
			else if (CurInventoryIndex == P_HOMING) {
				GEngine->AddOnScreenDebugMessage(2, 2.0f, FColor::Blue, FString::Printf(TEXT("유도탄 발사 %d발 남음"), HadProjectileNum[CurInventoryIndex]));
			}
			else if (CurInventoryIndex == P_EMP) {
				GEngine->AddOnScreenDebugMessage(2, 2.0f, FColor::Blue, FString::Printf(TEXT("EMP 발사 %d발 남음"), HadProjectileNum[CurInventoryIndex]));
			}
			bIsMyTurn = false;
		}
...
void ALOTMultiPlayer::NextWeapon()
{
	...
	if (CurInventoryIndex == P_COMMON) {
		GEngine->AddOnScreenDebugMessage(2, 2.0f, FColor::Blue, FString::Printf(TEXT("일반탄")));
	}
	else if (CurInventoryIndex == P_ARMORPIERCE) {
		GEngine->AddOnScreenDebugMessage(2, 2.0f, FColor::Blue, FString::Printf(TEXT("관통탄 %d"), HadProjectileNum[CurInventoryIndex]));
	}
	else if (CurInventoryIndex == P_HOMING) {
		GEngine->AddOnScreenDebugMessage(2, 2.0f, FColor::Blue, FString::Printf(TEXT("유도탄 %d"), HadProjectileNum[CurInventoryIndex]));
	}
	else if (CurInventoryIndex == P_EMP) {
